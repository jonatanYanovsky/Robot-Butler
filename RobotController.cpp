...
// assume we are in the charging station
void exitChargingStation() {
	...
	while ((sonarDistanceValues[0] < 800) && (sonarDistanceValues[7] < 800)) {
		...
		msg.linear.y = 0;
		msg.linear.z = 0;
		msg.angular.x = 0; // purposely go fast to get away from the charging station 
		msg.angular.y = 0;
		msg.angular.z = 0;
		pub.publish(msg);
		rate.sleep();
...
void poseCallback(const nav_msgs::Odometry::ConstPtr& msg) { 
	...
	if (i == 0) {
		...
		velocity = (double)distance * 10; // 10 is ros::Rate rate(10);
...
void sonarCallback(const sensor_msgs::PointCloud::ConstPtr& msg2) { 
	...
	for (int k=0; k<8; k++){
		xsonarVal = 1000 * msg2->points[k].x;
		ysonarVal = 1000 * msg2->points[k].y;
